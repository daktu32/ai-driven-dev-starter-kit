...
class WebNextjsPlugin {
    ...
    async applyProjectSpecificConfig(options, context, warnings) {
        ...
        if (await context.fileSystem.exists(packageJsonPath)) {
            ...
            await context.fileSystem.writeFile(packageJsonPath, JSON.stringify(packageJson, null, 2));
        }
        // 設定ファイルの生成
...
module.exports = {
  ...
  plugins: [],
            ...
            await context.fileSystem.writeFile(path.join(options.targetPath, 'tailwind.config.js'), tailwindConfig);
  ...
  plugins: {
    ...
    autoprefixer: {},
            ...
            await context.fileSystem.writeFile(path.join(options.targetPath, 'postcss.config.js'), postcssConfig);
        }
        // Jest設定
        if (options.options.testing) {
...
module.exports = createJestConfig(customJestConfig)`;
            await context.fileSystem.writeFile(path.join(options.targetPath, 'jest.config.js'), jestConfig);
            const jestSetup = `import '@testing-library/jest-dom'`;
            await context.fileSystem.writeFile(path.join(options.targetPath, 'jest.setup.js'), jestSetup);
        }
        // .env.local.example
...
`;
        ...
        await context.fileSystem.writeFile(path.join(options.targetPath, '.env.local.example'), envContent);
    }
