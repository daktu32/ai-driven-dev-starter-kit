...
class TestcleanServer {
  ...
  async start(): Promise<void> {
    ...
    logger.info('Starting test-clean MCP server...');
    
    await this.server.connect(transport);
    
    logger.info('test-clean MCP server started successfully');
  }
}

// Start the server
if (import.meta.url === `file://${process.argv[1]}`) {
  const server = new TestcleanServer();
