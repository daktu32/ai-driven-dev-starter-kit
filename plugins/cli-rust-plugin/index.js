...
class CliRustPlugin {
    ...
    async generateCargoToml(options, context) {
        const packageName = options.projectName
            .toLowerCase()
            .replace(/[^a-z0-9-_]/g, '-')
            .replace(/^[0-9]/, '_');
...
debug = true
`;
        await context.fileSystem.writeFile(path.join(options.targetPath, 'Cargo.toml'), cargoToml);
    }
...
`;
        await context.fileSystem.writeFile(path.join(options.targetPath, 'rust-toolchain.toml'), rustToolchain);
        // .cargo/config.toml
...
fmt = "fmt -- --check"
`;
        await context.fileSystem.writeFile(path.join(options.targetPath, '.cargo', 'config.toml'), cargoConfig);
        // Makefile
...
*~
`;
        await context.fileSystem.writeFile(path.join(options.targetPath, '.gitignore'), gitignore);
    }
    async generateGithubActions(options, context) {
    ...
    - name: Upload artifacts
      ...
      with:
        ...
        path: |
          ...
          !target/release/*.d
`;
        await context.fileSystem.writeFile(path.join(options.targetPath, '.github', 'workflows', 'ci.yml'), ciWorkflow);
        // Release workflow
...
`;
        await context.fileSystem.writeFile(path.join(options.targetPath, '.github', 'workflows', 'release.yml'), releaseWorkflow);
    }
