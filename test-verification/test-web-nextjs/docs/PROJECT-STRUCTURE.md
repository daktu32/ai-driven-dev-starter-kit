...
| パス | 役割 | 説明 |
|------|------|------|
| `src/index.ts` | エントリーポイント | Next.js アプリケーションのルートコンポーネント、レイアウト設定 |
| `src/components/` | components | UI コンポーネント、ページコンポーネント、共通コンポーネント |
| `src/pages/` | pages | ページコンポーネント、API エンドポイント、ルーティング |
...
**📌 重要**: この構造は test-web-nextjs の開発効率と品質向上のために設計されています。各ディレクトリ・ファイルの役割を理解して効果的に活用してください。