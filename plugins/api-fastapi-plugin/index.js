...
class ApiFastapiPlugin {
    ...
    async generateScaffold(template, options, context) {
        ...
        try {
            ...
            await context.templateProcessor.processTemplateDirectory(template.templatePath, options.targetPath, variables);
            // 生成されたファイルリストを収集
    ...
    async collectGeneratedFiles(targetPath, fileList, context) {
    ...
    async applyProjectSpecificConfig(options, context, warnings) {
    ...
    async generateRequirementsTxt(options, context) {
...
python-dotenv>=1.0.0
`;
        await context.fileSystem.writeFile(path.join(options.targetPath, 'requirements.txt'), requirements);
    }
    async generatePyprojectToml(options, context) {
...
line_length = 88
`;
        await context.fileSystem.writeFile(path.join(options.targetPath, 'pyproject.toml'), pyproject);
    }
    async generateConfigFiles(options, context, warnings) {
        // .env.example
...
`;
        ...
        await context.fileSystem.writeFile(path.join(options.targetPath, '.env.example'), envContent);
        // Makefile
...
`;
        await context.fileSystem.writeFile(path.join(options.targetPath, 'Makefile'), makefile);
    }
    async generateDockerFiles(options, context) {
        // Dockerfile
...
`;
        await context.fileSystem.writeFile(path.join(options.targetPath, 'Dockerfile'), dockerfile);
        // docker-compose.yml
...
`;
        ...
        await context.fileSystem.writeFile(path.join(options.targetPath, 'docker-compose.yml'), dockerCompose);
        // .dockerignore
...
.DS_Store
`;
        await context.fileSystem.writeFile(path.join(options.targetPath, '.dockerignore'), dockerignore);
    }
    prepareNextSteps(options) {
        const steps = [
            {
