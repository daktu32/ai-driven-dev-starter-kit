/**
 * Example Resource for e2e-test-mcp-server
...
export class ExampleResourceProvider {
  private resources: ExampleResource[] = [
    {
      ...
      description: 'First example resource for e2e-test-mcp-server',
      mimeType: 'text/plain',
    },
  ];
  ...
  async getResource(uri: string): Promise<string> {
    ...
    return `Content of ${resource.name}: This is example data from e2e-test-mcp-server`;
  }
}